...
namespace UncomplicatedCustomItems.HarmonyElements.Patches
{
    ...
    internal static class RevolverCylinderPatch
    {
        ...
        {
            if (SummonedAPICustomItem.TryGet(__instance.ItemSerial, out var summonItem) && summonItem != null && summonItem.CustomItem is CustomWeapon cw)
            {
                __result = cw.MaxMagazineAmmo;
                __instance.ServerResync();
            }

            if (Utilities.TryGetSummonedCustomItem(__instance.Firearm.ItemSerial, out var customItem) && customItem != null && customItem.CustomItem.CustomData is WeaponData wd)
            {
