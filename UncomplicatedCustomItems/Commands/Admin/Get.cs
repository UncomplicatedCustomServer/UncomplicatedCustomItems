...
{
    internal class Get : Subcommand
    {
        public override string Name { get; } = "get";

        public override string Description { get; } = "Get info on a summoned custom item";

        public override string VisibleArgs { get; } = "<Item Serial>";

        public override int RequiredArgsCount { get; } = 1;

        public override string RequiredPermission { get; } = "uci.get";

        public override string[] Aliases { get; } = ["get"];
        ...
        public override bool Execute(ArraySegment<string> args, ICommandSender sender, out string response)
        {
            ...
            if (Utilities.TryGetSummonedCustomItem(ushort.Parse(args.At(0)), out SummonedCustomItem? customItem) && customItem != null)
            {
            ...
            if (SummonedAPICustomItem.TryGet(ushort.Parse(args.At(0)), out var baseCustomItem) && baseCustomItem != null)
            {
            ...
            response = $"{ushort.Parse(args.At(0))} is not a custom item serial.";
            return false;
        }
    }
}