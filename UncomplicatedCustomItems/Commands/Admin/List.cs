...
{
    internal class List : Subcommand
    {
        public override string Name { get; } = "list";

        public override string Description { get; } = "Lists every registered Custom Item";

        public override string VisibleArgs { get; } = string.Empty;

        public override int RequiredArgsCount { get; } = 0;

        public override string RequiredPermission { get; } = "uci.list";
        ...
        public override bool Execute(ArraySegment<string> args, ICommandSender sender, out string response)
        {
            ...
            foreach (CustomItem Item in CustomItem.List.OrderBy(item => item.Id))
            ...
            if (CustomItem.UnregisteredList.Count > 0)
            {
                ...
                foreach (CustomItem item in CustomItem.UnregisteredList.OrderBy(item => item.Id))
            ...
            foreach (CustomAction action in CustomAction.List.OrderBy(action => action.Id))
            ...
            if (CustomAction.UnregisteredList.Count > 0)
            {
                ...
                foreach (CustomAction action in CustomAction.UnregisteredList.OrderBy(action => action.Id))
