...
namespace UncomplicatedCustomItems.Events
{
    internal class ScpHandler
    {
        ...
        {
            if (SummonedAPICustomItem.TryGet(ev.Scp127Item.Serial, out var api) && api.CustomItem is CustomSCP127 customSCP127 && customSCP127.MuteVoiceLines)
                ev.IsAllowed = false;

            if (Utilities.TryGetSummonedCustomItem(ev.Scp127Item.Serial, out var item) && item.CustomItem.CustomItemType is CustomItemType.SCPItem && item.CustomItem.CustomData is SCP127Data scp127Data && scp127Data.MuteVoiceLines)
                ev.IsAllowed = false;
        }

        private static void On127GainingExperience(Scp127GainingExperienceEventArgs ev)
        {
            if (SummonedAPICustomItem.TryGet(ev.Scp127Item.Serial, out var api) && api.CustomItem is CustomSCP127 customSCP127 && !customSCP127.AllowXPGain)
                ev.IsAllowed = false;

            if (Utilities.TryGetSummonedCustomItem(ev.Scp127Item.Serial, out var item) && item.CustomItem.CustomItemType is CustomItemType.SCPItem && item.CustomItem.CustomData is SCP127Data scp127Data && !scp127Data.AllowXPGain)
                ev.IsAllowed = false;
        }
    }
}